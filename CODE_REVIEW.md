...
## 0. Session checkpoint (2026-05-19)
### Current status
- **XPB_DEBUG diagnostic image added (2026-05-19)**: Introduced a minimal Nano Every W5500 sanity firmware path (`env:XPB_DEBUG`) to isolate hardware wiring/transport from production protocol behavior. The image parks non-W5500 SPI CS lines, brings up static XPB IP, listens on UDP 8888, emits 1 Hz heartbeats to UDP 8889, and echoes inbound packets. This provides a direct go/no-go signal for W5500 wiring and basic UDP path health independent of `ExpansionBoard` protocol state logic.【F:platformio.ini】【F:src/exp-board/xpb_debug.cpp】【F:docs/PROJECT_HANDOFF.md】【F:README.md】
- **CC unicast-send instrumentation added (2026-05-19)**: `ClearCoreRTM` now records UDP unicast `endPacket()` failures (`unicastFailCount_`) and periodically emits `CCDBG;UCAST_FAIL=<n>;LAST=<0|1>` over the existing observer tee path (UDP 8889) while observer mode is active. This is a diagnostic-only change to localize whether `REQ:PROTO` retries are stalling at CC unicast send versus later receiver/state-machine stages; protocol payload and retry behavior are unchanged.【F:include/ClearCoreRTM.h†L362-L402】
- **CC unicast socket-recycle remediation added (2026-05-19)**: when `flushTx_()` sees `kUnicastFailRecycleThresh` (5) consecutive `endPacket()==0` results on the peer unicast send, it calls `udp_.stop(); udp_.begin(RtmNet::kUdpPort)` to force a fresh ARP probe for `peerIp_`, increments `unicastRecycleCount_`, and resets the streak. The 1 Hz `CCDBG` line now carries the new `RECYC=<n>` field so the observer trace can prove the recovery path fired. This directly targets the most-likely failure mode (silent unicast endPacket failures wedging the CC→XPB handoff) without changing protocol semantics. Build cost: +80 B flash (`clearcore` now 143,604 B / 42,060 B RAM).【F:include/ClearCoreRTM.h†L362-L420】
- **XPB W5500 SPI clock capped at 4 MHz (2026-05-19)**: Wireshark L2 capture during a Teensy-driven reset cycle proved CC was placing well-formed `REQ:PROTO` unicast frames on the wire to `10.0.0.11:8888` while XPB was emitting corrupted UDP TX (Len=120/206/256 payloads filled with AVR flash-pattern bytes `\x90\xXX`) and silently dropping the vast majority of inbound unicast — 21 CC bursts produced only 1 `XPBDBG;CC_REQ_PROTO_RX`, and the one reply CC did receive failed checksum. Root cause is W5500 SPI corruption on the XPB at the upstream 8 MHz default with current jumper-wire bring-up. Mitigation: `wiznet_SPI_settings` in the vendored Ethernet3 library now uses `RTM_W5500_SPI_HZ` (default 4 MHz, build-overridable). All three XPB envs build clean (`XPB` 46,406 B, `XPB_DEBUG` 15,438 B, `XPB_PROD_TRACE` 48,336 B).【F:lib/Ethernet3/src/utility/w5500.cpp†L23-L36】【F:lib/Ethernet3/RTM_PATCH_NOTES.md】
- **Bug #17 — XPB protocol-ready announcement was one-shot at boot**: XPB previously emitted `NOTICE;PROTO_READY` once during boot and then waited for `REQ:PROTO`. If that startup notice was dropped during early link/socket churn, bench traces could appear silent until later retries. Mitigation implemented: while `ProtoTxState::WaitingReq`, XPB now re-announces `NOTICE;PROTO_READY` every 2s; entering `WaitingReq` resets the resend timer for boot/reset/SD-recovery paths.【F:include/ExpansionBoard.h†L324-L331】【F:src/exp-board/ExpansionBoard.cpp†L247-L253】【F:src/exp-board/ExpansionBoard.cpp†L661-L670】
- **Link-guard refinement (2026-05-19 follow-up)**: the strict PHY-down guards were too aggressive for this bench failure mode and could suppress XPB response traffic when PHY status was transiently false. Updated behavior: `pumpRx_()` no longer hard-returns on PHY-down, and `flushTx_()` drops frames only when PHY is down **and** CC RX is stale (>3s). This keeps the anti-stall guard while allowing handoff traffic during noisy link-status windows.【F:include/ExpansionBoard.h†L491-L525】
- **Link-guard rollback (2026-05-19 same-day follow-up)**: after flash-and-test, the rig still showed CC-only `REQ:PROTO` with zero XPB frames, so TX drop-on-link logic was removed entirely from `flushTx_()`. Rationale: eliminate silent suppression from unreliable PHY status as a root cause while preserving all other handshake instrumentation. Residual risk: if the original W5500 stall mode reappears under true cable-loss, loop stalls may recur and need a different non-suppressing mitigation (e.g., bounded send cadence or socket watchdog).【F:include/ExpansionBoard.h†L491-L500】
- Build validation (no hardware run yet): `pio run -e XPB_PROD_TRACE` and `pio run -e XPB` both pass. Current XPB sizes after this change: `XPB_PROD_TRACE` flash 48,426 B / RAM 3,047 B; `XPB` flash 46,406 B / RAM 3,035 B.
- Build validation (this session): `pio run -e clearcore` passes after instrumentation. ClearCore size is flash 143,524 B / RAM 42,060 B.
- Hardware validation is pending; highest-risk unvalidated path is cold-boot handoff under intermittent link where CC repeats `REQ:PROTO` while XPB is in `WaitingReq`.

### Next actions
1. Capture one cold-boot handoff trace with observer enabled and verify whether `CCDBG;UCAST_FAIL` increments during repeated `REQ:PROTO` bursts.
2. Run `python -m pytest test/scenarios/test_reset_reloads_protocol.py -v` and confirm full marker chain completion (`REQ:PROTO` -> `PR_BEG/PR_DAT/PR_END` -> `PROTO_RX=OK` -> `IDLE`).
3. If `UCAST_FAIL` stays zero while stalls persist, escalate to L2 packet capture (destination MAC on CC→XPB unicast) before any further firmware logic edits.
...
- Hardware validation after reconnecting Ethernet: pytest captured both boards on observer UDP 8889. Six scenarios passed; two failures were harness interpretation issues, not firmware regressions. [test_protocol_upload.py](test/scenarios/test_protocol_upload.py) now treats RUNNING/PAUSED/COMPLETED HBs as proof that a protocol was already loaded/executed, and [test_run_gate.py](test/scenarios/test_run_gate.py) now waits for `PROTO_RX=OK` plus the later IDLE HB before asserting gate behavior. Source log: [test/log/2026/05/15/103107_pytest.log](test/log/2026/05/15/103107_pytest.log).
- Harness hardening in progress: shared live-rig helpers centralize reset-to-IDLE, reload-chain classification, and transcript formatting; pytest markers now distinguish smoke/full/stateful/manual tiers; timestamped logs append command and result summaries; and the canonical operator procedure is documented in `docs/TEST_PROCEDURE.md`.
- Latest full-suite attempt showed the PC receiving CC heartbeats only while XPB frames stayed at zero and CC reported E-STOP. The smoke gate now stops the suite after any smoke failure or skip, and comms smoke logs endpoint frame counts before asserting.

### Next actions
1. Run collection/static validation for the hardened harness, then re-run `python -m pytest test/scenarios/ -v -m "not manual"` with `HLFB_QUICK` on SD.
2. Bench-test with CC+XPB on the router and PC unplugged: RUN/RST must still work.
3. Add manual-assisted protocol-swap, thermal, and power-loss/resume scenarios as follow-up coverage.
