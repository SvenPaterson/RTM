...
from test.rig.parser import HbFrame
...
def test_run_edge_cycles_state(monitor: Monitor, teensy: Teensy) -> None:
    ...
    # --- Phase 0: normalize ---------------------------------------------
    reset_to_idle(monitor, teensy, _log)
    ...
    if start_state not in ("IDLE", "PAUSED"):
        pytest.fail(
            ...
            "Self-normalization failed to leave the rig startable."
        )

    # --- Phase 2: RUN high #1 -> RUNNING ---------------------------------
    edge1_t = monitor.elapsed_ms
    teensy.set_run(1)
    running_hbs = wait_for_state(monitor, edge1_t, "RUNNING", RUN_SETTLE_S)
    if not running_hbs:
    ...
    teensy.set_run(0)
    paused_hbs = wait_for_state(monitor, edge2_t, "PAUSED", PAUSE_SETTLE_S)
    if not paused_hbs:
    ...
    teensy.set_run(1)
    running2 = wait_for_state(monitor, edge3_t, "RUNNING", RUN_SETTLE_S)
    if not running2:
    ...
    teensy.set_run(0)
    paused2 = wait_for_state(monitor, edge4_t, "PAUSED", PAUSE_SETTLE_S)
    if not paused2:
